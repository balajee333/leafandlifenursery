...
export default function DesignerPotsPage() {
  ...
  const designerPots = pots.filter((pot) => pot.category === "designer");

  const filtered = designerPots.filter((item) =>
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      <div className="text-center">
        ...
        <p className="mt-2 text-sm text-gray-500">
          Curated designer pots — single finish, fixed price.
        </p>
      </div>
      ...
      <div className="mt-8 flex flex-col gap-4 sm:items-center sm:justify-between">
        <div className="relative max-w-md w-full">
          ...
          <input
            type="text"
            placeholder="Search designer pots..."
            value={search}
      ...
      {filtered.length === 0 ? (
        <div className="mt-16 text-center text-gray-400">
          <p className="text-lg">No designer pots found</p>
          <button
            onClick={() => setSearch("")}
